...
install_venv() {
    ...
    if [ $? -ne 0 ]; then
        echo "python3-venv is still not available. Please install it manually."
        exit 1
    else
...
# Updated default: x.com, twitter.com
...
# New prompt for Owner ID. If provided, only that user will be allowed to use slash commands.
read -rp "Enter your Discord Owner ID (leave blank to allow everyone to use slash commands): " OWNER_ID
